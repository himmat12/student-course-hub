...
ON
    p.ProgrammeLeaderID = s.StaffID WHERE p.ProgrammeName LIKE :query  ORDER BY p.ProgrammeID DESC;";
...
INNER JOIN staff AS s
ON
    p.ProgrammeLeaderID = s.StaffID WHERE p.ProgrammeName LIKE :query AND LevelID = :levelID ORDER BY p.ProgrammeID DESC;";
